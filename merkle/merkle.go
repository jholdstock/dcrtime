...
func VerifyAuthPath(mb *Branch, digest *[sha256.Size]byte) (*[sha256.Size]byte, error) {
	...
	// Validate that we consumed all bits and bobs.  The flags must be exactly
	// the bits the traversal consumed, zero padded to a whole number of bytes;
	// anything else is a second encoding of the same branch.
	if int(m.bitsUsed+7)/8 != len(mb.Flags) {
		return nil, fmt.Errorf("did not consume all flag bits")
	}
	for i := m.bitsUsed; i < uint32(len(m.bits)); i++ {
		if m.bits[i] != 0 {
			return nil, fmt.Errorf("non-zero padding in flag bits")
		}
	}
	...
	// Bind the branch to the digest being verified.  A branch generated by
	// AuthPath marks exactly the leaves equal to the requested digest, so any
	// other matched leaf means this branch proves something else.
	if len(m.hashes) == 0 {
		return nil, fmt.Errorf("branch proves no leaf, expected %x", *digest)
	}
	for _, h := range m.hashes {
		if h != *digest {
			return nil, fmt.Errorf("branch proves leaf %x, expected %x", h,
				*digest)
		}
	}
